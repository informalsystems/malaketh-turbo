...
fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::Generate { txs_per_block, num_blocks } => generate_all(txs_per_block, num_blocks),
...
fn generate_all(txs_per_block: usize, num_blocks: usize) -> Result<()> {
    ...
    let signers: Vec<LocalSigner<SigningKey>> = TEST_MNEMONICS.iter()
        .map(|&mnemonic| {
    ...
    let signer_addresses: Vec<Address> = signers.iter()
...
fn generate_test_blocks(
    ...
    println!("Writing {} blocks to {} with {} transactions each...", num_blocks, output_file, txs_per_block);
    ...
    for block_idx in 0..num_blocks {
        ...
        println!("Wrote block {} of {} to {} (size: {:.2} MB, {} transactions)", 
            block_idx + 1,
            num_blocks,
            output_file,
...
fn verify_block_logs(log_file: &str) -> eyre::Result<()> {
    ...
    for line in reader.lines() {
        ...
        if line.contains("Executing block") {
            ...
            if let Some(height) = line.split_whitespace()
        ...
        // Check for transaction signer
            ...
            if let Some(signer) = line.split_whitespace()
                .last()
            ...
            if parts.len() >= 2 {
                if let Ok(balance) = U256::from_str(parts.last().unwrap_or(&"0")) {
                    if line.starts_with("Initial") {
                        ...
                        if let Some(exec) = executions.iter_mut().rev()
                            .find(|e| e.height == current_height && e.signer == current_signer.unwrap()) {
    ...
    let max_height = executions.iter()
    ...
    let executions: Vec<_> = executions.into_iter()
    ...
    for exec in &executions {
        if exec.height != exec.block_number {
            println!("❌ Height/block number mismatch at height {}: block number is {}", 
    ...
    for exec in &executions {
        balance_sequences
            .entry(exec.signer)
            .or_default()
            .push((exec.height, exec.initial_balance, exec.final_balance));
    }
        ...
        for (height, initial, final_bal) in &sorted_sequence {
            ...
            if let Some((last_height, last_balance)) = last_final_balance {
                if *initial != last_balance {
                    ...
                    println!("    Final balance at height {}: {}", last_height, last_balance);
                    println!("    Initial balance at height {}: {}", height, initial);
                    if height > &(last_height + 1) {
                        println!("    Note: {} block(s) between these heights", height - last_height - 1);
                    }
                }
            }
            ...
            if final_bal > initial {
                println!("❌ Unexpected balance increase at height {}: {} -> {}", 
...
fn verify_rpc_state() -> Result<()> {
    ...
    fn rpc_call(method: &str, params: serde_json::Value) -> Result<serde_json::Value> {
        ...
        let response = client.post("http://127.0.0.1:8545")
        ...
        if let Some(block) = block.as_object() {
            ...
            if let Some(txs) = block.get("transactions").and_then(|t| t.as_array()) {
                if let Some(first_tx) = txs.first() {
                    ...
                    let initial_balance = rpc_call(
                        "eth_getBalance",
                        serde_json::json!([signer, prev_block])
                    )?;
                    
                    let final_balance = rpc_call(
                        "eth_getBalance",
                        serde_json::json!([signer, curr_block])
                    )?;
                    
                    let initial_balance = U256::from_str_radix(
                        &initial_balance.as_str().unwrap_or("0x0")[2..],
                        16
                    )?;
                    
                    let final_balance = U256::from_str_radix(
                        &final_balance.as_str().unwrap_or("0x0")[2..],
                        16
                    )?;
    ...
    if !missing_blocks {
        println!("✅ All blocks from height 1 to {} were executed", 10.min(latest_block));
    }
